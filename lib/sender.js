...
var authenticateMethod = Sender.prototype.authenticate = function (options, callback) {
    var self = this;
    options = options || {};
    ...
    requestBody = 
        ...
        Constants.JSON_CLIENT_ID + "=" + options.client_id + "&" +
        Constants.JSON_CLIENT_SECRET + "=" + options.client_secret;
...
Sender.prototype.send = function (message, registrationId, callback) {
   if (!this.access_token || (this.access_token_expiration_date < new Date())) {
        return callback(new Error("Sender is not authenticated."));
   }    
    ...
    post_req = https.request(post_options, function (res) {
        ...
        res.on('end', function () {
            ...
            if (statusCode === 503) {
                ...
                // Non recoverable error
                return callback(new Error('Authentication expired.'));
            } else if (statusCode !== 200) {
                console.log(data);
                if (data && data.error) {
